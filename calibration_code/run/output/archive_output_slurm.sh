...
#  cd /scratch/daint/ksilver/calibration/run/$param$min/output
  ...
  for year in 1994 1995 1996 1997 1998
  do
    tar -cf $runpath/$param$min/output/"$param$min"_out01_$year.tar $runpath/$param$min/output/out01/lffd$year*
    tar -cf $runpath/$param$min/output/"$param$min"_out02_$year.tar $runpath/$param$min/output/out02/lffd$year*
  done

  rsync -a /scratch/daint/ksilver/calibration/run/$param$min/output/*.tar /project/ch4/ksilver/cosmo5_calibration/output/$param$min
  echo Finished with $counter of 129
  counter=$((counter+1))
# Then the maximum
  ...
  for year in 1994 1995 1996 1997 1998
  do
    tar -cf "$param$max"_out01_$year.tar ./out01/lffd$year*
    tar -cf "$param$max"_out02_$year.tar ./out02/lffd$year*
  done

  rsync -a *.tar /project/ch4/ksilver/cosmo5_calibration/output/$param$max
  echo Finished with $counter of 129
  counter=$((counter+1))
  done
...
for param1 in rl e q u f t ra s
  ...
  for param2 in rl e q u f t ra s
    ...
    if [ "$i1" -lt "$i2" ]
      ...
      for year in 1994 1995 1996 1997 1998
      do
        tar -cf ${val1}_${val2}_out01_$year.tar ./out01/lffd$year*
        tar -cf ${val1}_${val2}_out02_$year.tar ./out02/lffd$year*
      done

      rsync -a *.tar /project/ch4/ksilver/cosmo5_calibration/output/${val1}_${val2}
      echo Finished with $counter of 129
      counter=$((counter+1))
